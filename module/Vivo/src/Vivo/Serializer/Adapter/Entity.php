...
// 					if (substr($name, 0, 2) != '__') //@fixme: proc?
				...
				return $out.$indent.'}';
		default:
			throw new \Vivo\Exception("Unsupported type $type of value $value");
		}
	}
	...
	public function unserializeRun(&$str, &$pos = 0) {
		switch ($type = Text::readWord($str, $pos)) {
			...
			case 'string':
				...
				$len = strlen($str);
				$start = $pos;
				while (!($str{$pos} == '"' && ($str{$pos - 1} != '\\' || $str{$pos - 2} == '\\')) && ($pos < $len))
				$pos++;
				$str2 = str_replace("\\\"", "\"", substr($str, $start, ($pos++) - $start));
			...
			case ')': // end of array
				return self::EOA;
			default:
				throw new \Vivo\Exception("Undefined type '$type' at position $pos: \"".substr(substr($str, $pos), 20)."...\"");
		}
	}
}
